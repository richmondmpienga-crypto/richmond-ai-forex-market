...
const stopLossPips = isDeriv
  ? Math.abs(entry - stopLoss)
  : Math.abs(entry - stopLoss) / pipSize;

const tp1Pips = isDeriv
  ? Math.abs(tp1 - entry)
  : Math.abs(tp1 - entry) / pipSize;

const tp2Pips = isDeriv
  ? Math.abs(tp2 - entry)
  : Math.abs(tp2 - entry) / pipSize;

const distanceLabel = isDeriv ? "points" : "pips";
stopLossValue.textContent =
    confirmedSignal === "WAIT"
        ? "--"
       : `${formatPrice(stopLoss)} (${stopLossPips.toFixed(1)} ${distanceLabel})`;

tp1Value.textContent =
    confirmedSignal === "WAIT"
        ? "--"
       : `${formatPrice(tp1)} (${tp1Pips.toFixed(1)} ${distanceLabel})`;

tp2Value.textContent =
    confirmedSignal === "WAIT"
        ? "--"
       : `${formatPrice(tp2)} (${tp2Pips.toFixed(1)} ${distanceLabel})`;
